...
class BEGAN:
    """
   models.BEGAN(dataset, folder_name='began', **kwargs)
   ...
   ----------
    start_lr : float
       The initial learning rate to use. Default is 0.0001.
    num_epochs : int
    ...
    bz : int
        ...
        Default size is 64.
    num_hidden : int
        Corresponds to Nh in Berthelot et al, the number of units to
        use in hidden representation of image. Default is 64.
    z_var : Function
    ...
    gamma : float
        ...
        sample from 0.5 up.
    num_filters : int
        number of filters ``n`` to use to start. Filters increase linearly
        with each upsample. Default is 64.
    k_t : float
        Initial k_t from Berthelot et al. Default is 0. Note ``k_t`` is updated
        each epoch, so ``k_t`` varies by epoch.
    offset : int
        Berthelot et al train by linearly increasing number of filters
        ...
        Default is 5 (no increase in filters).
    num_examples : int
    ...
    seed : int
        Seed to use for random number (z_vector) generation during test time. Default is 115.
    n_inter : int
        Number of steps to use when generating interpolation test figure.
   References
   ----------
   .. [1] David Berthelot, Thomas Schumm, Luke Metz (2017):
          BEGAN: Boundary Equillibrium Generative Adversarial Networks. arXiv.
          https://arxiv.org/abs/1703.10717
   """

    def __init__(self, dataset, folder_name='began', **kwargs):
