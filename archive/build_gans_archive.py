...
# We split apart conv from batch norm so that we can access weights for the discriminator later on
...
def make_train_fns():
    ...
    # y_var is in format [batchsize, categories, 1] and is flattened out in buid_generator
    discriminator, input_var = build_discriminator(y_var)
    generator, z_var = build_generator(discriminator, y_3)
    
    
    #convert y_var to 4D tensor
    real_out = lasagne.layers.get_output(discriminator['out'], {discriminator['input']:input_var, discriminator['input_y']:T.reshape(T.extra_ops.repeat(y_3, 32*32, axis=2), (64, 18, 32, 32))})
    real_out_yfake = lasagne.layers.get_output(discriminator['out'], {discriminator['input']:input_var, discriminator['input_y']:T.reshape(T.extra_ops.repeat(y_fake, 32*32, axis=2), (64, 18, 32, 32))})

    ims = lasagne.layers.get_output(generator['gen_out'])
    fake_out = lasagne.layers.get_output(discriminator['out'], {discriminator['input']:ims, discriminator['input_y']:T.reshape(T.extra_ops.repeat(y_3, 32*32, axis=2), (64, 18, 32, 32))})
...
def build_generator(discriminator, y_var):
    ...
    # Append Discriminator to Generator
#    lrelu = lasagne.nonlinearities.LeakyRectify(0.2)
#    y_v_d = T.reshape(T.extra_ops.repeat(y_var, 32*32, axis=2), (64, 18, 32, 32))
#    
#    input_shape = (None, 18, 32, 32)
#    name = 'input_y'
#    generator[name] = lasagne.layers.InputLayer(shape=input_shape, input_var=y_v_d)
#    output_dims = lasagne.layers.get_output_shape(generator[name])
#    
#    prev_name = 'gen_out'
#    name = 'pad1'
...
#                    filter_size, stride=2, pad='valid', W=discriminator[name].W, 
#                        b=discriminator[name].b, nonlinearity=lrelu)
#    prev_output_dims = output_dims
#    output_dims = lasagne.layers.get_output_shape(generator[name])
#    details.append([name, str(prev_output_dims), str((num_filters, prev_num_filters, filter_size, filter_size)),
#                    str(output_dims)])
...
#    prev_name = name
#    name = 'concat'
#    generator[name] = lasagne.layers.ConcatLayer([generator[prev_name], generator['input_y']])
#    prev_output_dims = output_dims
#    output_dims = lasagne.layers.get_output_shape(generator[name])
...
#    filter_size = 4
#    prev_num_filters = lasagne.layers.get_output_shape(generator[prev_name])[1]
#    generator = batch_conv(generator, prev_name, name, num_filters, filter_size, 
#                           2, 'valid', discriminator[name].W, lrelu)    
#    prev_output_dims = output_dims
#    output_dims = lasagne.layers.get_output_shape(generator[name])
#    details.append([name, str(prev_output_dims), str((num_filters, prev_num_filters, filter_size, filter_size)),
#                    str(output_dims)])
...
#    prev_num_filters = lasagne.layers.get_output_shape(generator[prev_name])[1]
#    generator[name] = lasagne.layers.Conv2DLayer(generator[prev_name], num_filters,
#                            filter_size, stride=1, pad='valid', W=discriminator[name].W, b=discriminator[name].b,
...
def build_discriminator(y_var=None):
    ...
    # T.reshape(T.extra_ops.repeat(y_var, 32*32, axis=2), (64, 18, 32, 32))
