...
def iterate_membatches(inputs, targets, batchsize, dataset_loader, li, shuffle=False):
    if shuffle:
        indices = np.arange(len(inputs))
        np.random.shuffle(indices)
    for start_idx in range(0, len(inputs) - batchsize + 1, batchsize):
        if shuffle:
            ...
            excerpt = slice(start_idx, start_idx + batchsize)
        yield dataset_loader(inputs[excerpt], inputs[excerpt].shape[0], li), targets[excerpt].astype(np.float32)


# Generates images to be loaded into memory
# This version uses unconditional data
def iterate_membatches_unconditional(inputs, batchsize, dataset_loader, li, shuffle=False):
    if shuffle:
        indices = np.arange(len(inputs))
        np.random.shuffle(indices)
    for start_idx in range(0, len(inputs) - batchsize + 1, batchsize):
